...
### 1. `users`
Stores all system users (Admin, Cashier, Supervisor).
...
| `role` | VARCHAR(50) (PHP Enum: 'Admin','Cashier','Supervisor') | NOT NULL, DEFAULT 'Cashier' | |
| `status` | VARCHAR(50) (PHP Enum: 'Active','Inactive') | NOT NULL, DEFAULT 'Active' | |
...
| `status` | VARCHAR(50) (PHP Enum: 'Active','Low Stock','No Stock','Disabled') | NOT NULL, DEFAULT 'Active' | |
...
| `doc_type` | VARCHAR(50) (PHP Enum: 'S.I.','D.R.','C.I.') | NULLABLE | Sales Invoice / Delivery Receipt / Charge Invoice |
| `status` | VARCHAR(50) (PHP Enum: 'Completed','Refund','Return','Void','Pending','Deposit','Paid','Restocked','Damaged','Security Alert') | NOT NULL | |
| `type` | VARCHAR(50) (PHP Enum: 'sale','reservation','inventory','system') | NULLABLE | Transaction category |
...
| `price_tier` | VARCHAR(50) (PHP Enum: 'price1','price2') | DEFAULT 'price1' | Which price was used |
...
| `doc_type` | VARCHAR(50) (PHP Enum: 'S.I.','D.R.','C.I.') | NOT NULL | |
| `cashier_id` | BIGINT UNSIGNED | FK → `users.id`, NOT NULL | |
| `status` | VARCHAR(50) (PHP Enum: 'Pending','Urgent','Approved','Rejected') | NOT NULL, DEFAULT 'Pending' | |
| `created_at` | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | |
| `updated_at` | TIMESTAMP | ON UPDATE CURRENT_TIMESTAMP | |
...
| `payment_type` | VARCHAR(50) (PHP Enum: 'deposit50','full') | NOT NULL | 50% deposit or full payment |
...
| `status` | VARCHAR(50) (PHP Enum: 'Pending','Completed','Cancelled') | NOT NULL, DEFAULT 'Pending' | |
| `created_at` | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP | |
| `updated_at` | TIMESTAMP | ON UPDATE CURRENT_TIMESTAMP | |
...
### 14. `notifications`
System-generated alerts (low stock, transactions).

| Column | Type | Constraints | Notes |
|---|---|---|---|
| `id` | BIGINT UNSIGNED | PK, AUTO_INCREMENT | |
| `type` | VARCHAR(50) (PHP Enum: 'low_stock','transaction','reservation') | NOT NULL | |
...
TABLES:
- users (id PK, employee_id UNIQUE, name, real_name, email, username UNIQUE, password, pin, role VARCHAR[PHP Enum: Admin/Cashier/Supervisor], status VARCHAR[PHP Enum: Active/Inactive], profile_photo, timestamps)
...
- products (id PK, parent_product_id FK->products NULLABLE self-ref, name, chinese_name, part_no UNIQUE, category_id FK->categories, address, stock INT, alert_limit INT, price1 DECIMAL, price2 DECIMAL, sales_count INT, status VARCHAR[PHP Enum: Active/Low Stock/No Stock/Disabled], is_dead_stock BOOL, damaged INT, variant_options VARCHAR, notes TEXT, timestamps)
...
- transactions (id PK, si_no UNIQUE, or_no, date DATETIME, customer_id FK->customers, cashier_id FK->users, total_qty INT, amount DECIMAL, amount_tendered DECIMAL, payment_method, doc_type VARCHAR[PHP Enum: S.I./D.R./C.I.], status VARCHAR[PHP Enum: Completed/Refund/Return/Void/Pending/Deposit/Paid/Restocked/Damaged/Security Alert], type VARCHAR[PHP Enum: sale/reservation/inventory/system], refund_reason, void_reason, action_type, inv_action, approver_id FK->users NULLABLE, approval_code, order_ref, internal_notes TEXT, timestamps)
- transaction_items (id PK, transaction_id FK->transactions CASCADE, product_id FK->products, qty INT, price DECIMAL, price_tier VARCHAR[PHP Enum: price1/price2], unit VARCHAR, created_at)
- pending_purchase_orders (id PK, po_no UNIQUE, date DATETIME, customer_id FK->customers, items_count INT, total DECIMAL, doc_type VARCHAR[PHP Enum: S.I./D.R./C.I.], cashier_id FK->users, status VARCHAR[PHP Enum: Pending/Urgent/Approved/Rejected], timestamps)
- pending_po_items (id PK, pending_po_id FK->pending_purchase_orders CASCADE, product_id FK->products, qty INT, price DECIMAL)
- reservations (id PK, order_no UNIQUE, customer_id FK->customers, email, notes TEXT, payment_method, payment_type VARCHAR[PHP Enum: deposit50/full], deposit DECIMAL, total DECIMAL, date DATE, pickup_date DATE, pickup_time TIME, reserved_by_id FK->users, fulfilled_by_id FK->users NULLABLE, status VARCHAR[PHP Enum: Pending/Completed/Cancelled], timestamps)
- reservation_items (id PK, reservation_id FK->reservations CASCADE, product_id FK->products, qty INT, price DECIMAL)
- notifications (id PK, type VARCHAR[PHP Enum: low_stock/transaction/reservation], sub_type, title, message TEXT, link, product_id FK->products NULLABLE, transaction_id FK->transactions NULLABLE, is_read BOOL, user_id FK->users NULLABLE, created_at)
